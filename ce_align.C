...
;
///////////////////////////////////////////////////////////////////////////
double ce_1(char *name1, char *name2, XYZ *ca1, XYZ *ca2, int nse1, int nse2,
		int *align_se1, int *align_se2, int& lcmp, int winSize_, double rmsdThr,
		double rmsdThrJoin, double d_[20], int isPrint,
		int *r_aln_len, int *r_gaps, float *r_rmsd) {

	int distAll = 0;

	if (isPrint == 2)
		printf("CE Algorithm, version 1.00, 1998.");
	else if (isPrint == 1)
		for (int ip = 0; ip < 85; ip++)
			printf("-");
	...
	double d, dd, d1, d2, dx, dy, dz, score, score0, score1, score2, rmsd,
			rmsdNew, z;
	...
	int *traceIterLevel = new int[traceMaxSize], *traceIndex =
			new int[traceMaxSize];
	double **traceScore;
	int **traceUsage;
	...
	int *bestTrace1 = new int[traceMaxSize],
	...
	for (ise1 = 0; ise1 < nse1; ise1++)
		for (ise2 = 0; ise2 < nse1; ise2++)
			*(dist1[ise1] + ise2) =
					ca1[ise1].X < 1e10 && ca1[ise2].X < 1e10 ?
							ca1[ise1].dist(ca1[ise2]) : 2e10;

	for (ise1 = 0; ise1 < nse2; ise1++)
		for (ise2 = 0; ise2 < nse2; ise2++)
			*(dist2[ise1] + ise2) =
					ca2[ise1].X < 1e10 && ca2[ise2].X < 1e10 ?
							ca2[ise1].dist(ca2[ise2]) : 2e10;
	...
	for (int iter = 0; iter < nIter; iter++) {
		...
		for (int ise1_ = ise11; ise1_ < ise12; ise1_++) {
			for (int ise2_ = ise21; ise2_ < ise22; ise2_++) {
				...
				if (iter == 0
						&& (ise1 > nse1 - winSize * (nBestTrace - 1)
								|| ise2 > nse2 - winSize * (nBestTrace - 1)))
					continue;
				...
				while (nTrace > 0) {
					...
					if (isTraceUp) {
						...
						for (int it = 0; it < iterDepth; it++) {
							...
							if (!distAll) {
								// (winSize) "best" dist
								for (int itrace = 0; itrace < nTrace;
										itrace++) {
									score += fabs(
											*(dist1[trace1[itrace]] + mse1)
													- *(dist2[trace2[itrace]]
															+ mse2));
									score +=
											fabs(
							...
							if (nTrace > nBestExtTrace
									|| (nTrace == nBestExtTrace
											&& score2 < bestExtScore)) {
								bestExtScore = score2;
								nBestExtTrace = nTrace;
								traceIndex_ = it;
								*(traceScore[nTrace - 1] + traceIndex_) =
										score1;
							}
							...
							score2 = ((
									nTrace > 1 ?
											*(traceScore[nTrace - 2]
													+ traceIndex[nTrace - 1]) :
											score0) * a[nTrace - 1]
									+ score1 * (a[nTrace] - a[nTrace - 1]))
									/ a[nTrace];

							if (score2 > rmsdThrJoin)
								traceIndex_ = -1;
							else
								traceTotalScore = *(traceScore[nTrace - 1]
										+ traceIndex_) = score2;
						} else {
						...
						if (iter == 0)
							if (nTrace > bestTracesN[newBestTrace]
									|| (nTrace == bestTracesN[newBestTrace]
											&& bestTracesScores[newBestTrace]
													> traceTotalScore)) {
								for (int itrace = 0; itrace < nTrace;
										itrace++) {
									*(bestTraces1[newBestTrace] + itrace) =
											trace1[itrace];
									*(bestTraces2[newBestTrace] + itrace) =
											trace2[itrace];
	...
	if (nBestTrace > 0) {
		...
		for (int ir = 0; ir < nBestTraces; ir++) {
			...
			rmsdNew = calc_rmsd(strBuf1, strBuf2, bestTracesN[ir] * winSize,
					d_);
		...
		int *traceLen = new int[traceMaxSize], *bestTraceLen =
				new int[traceMaxSize];
		...
		for (int it = 0; it < nTrace;) {
			...
			for (jt = it + 1; jt < nTrace; jt++) {
				if (trace1[jt] - trace1[jt - 1] - traceLen[jt - 1] != 0
						|| trace2[jt] - trace2[jt - 1] - traceLen[jt - 1] != 0)
					break;
				cSize += traceLen[jt];
			}
		...
		for (int it = 1; it < nBestTrace; it++) {
			int igap;
			if (bestTrace1[it] - bestTrace1[it - 1] - bestTraceLen[it - 1] > 0)
				igap = 0;
			if (bestTrace2[it] - bestTrace2[it - 1] - bestTraceLen[it - 1] > 0)
				igap = 1;
				...
				for (int idep = 1; idep <= winSize / 2; idep++) {
					...
					for (jt = 0; jt < nBestTrace; jt++) {
						for (i = 0; i < traceLen[jt]; i++) {
							if (ca1[trace1[jt] + i].X > 1e10
									|| ca2[trace2[jt] + i].X > 1e10)
								goto bad_ca;
		...
		if (z >= zThr) {
			...
			while (nAtom < strLen * 0.95
					|| (isRmsdLenAssigned && rmsd < rmsdLen * 1.1
							&& nAtomPrev != nAtom)) {
				nAtomPrev = nAtom;
				oRmsdThr += 0.5;
					...
					for (ise2 = 0; ise2 < nse2; ise2++) {
						*(mat[ise1] + ise2) = -0.001;
						...
						if (ca1[ise1].X < 1e10 && ca3[ise2].X < 1e10)
							*(mat[ise1] + ise2) = oRmsdThr
									- ca1[ise1].dist(ca3[ise2]);
					}
				}
				dpAlign(mat, nse1, nse2, align_se1, align_se2, lcmp, 5.0, 0.5,
						0, 0);
				...
				for (int ia = 0; ia < lcmp; ia++)
					if (align_se1[ia] != -1 && align_se2[ia] != -1) {
						if (ca1[align_se1[ia]].X < 1e10
								&& ca2[align_se2[ia]].X < 1e10) {
			...
			for (int k = 0; k < nBestTrace; k++) {
				...
				if (k < nBestTrace - 1) {
					if (bestTrace1[k] + bestTraceLen[k] != bestTrace1[k + 1])
						for (int l = bestTrace1[k] + bestTraceLen[k];
								l < bestTrace1[k + 1]; l++) {
							align_se1[lcmp] = l;
							align_se2[lcmp] = -1;
							lcmp++;
					...
					if (bestTrace2[k] + bestTraceLen[k] != bestTrace2[k + 1])
						for (int l = bestTrace2[k] + bestTraceLen[k];
								l < bestTrace2[k + 1]; l++) {
							align_se1[lcmp] = -1;
							align_se2[lcmp] = l;
							lcmp++;
		...
		long time_q = (time2.tms_stime - time1.tms_stime + time2.tms_utime
				- time1.tms_utime);

		if (isPrint) {
			printf(
					"Alignment length = %d Rmsd = %.2fA Z-Score = %.1f Gaps = %d(%.1f%%) CPU = %ds ",
					nAtom, rmsd, z, nGaps, nGaps * 100.0 / nAtom,
...
///////////////////////////////////////////////////////////////////////////
double trace_rmsd(int *trace1, int *trace2, int nTrace, int winSize, XYZ *ca1,
		XYZ *ca2) {
...
///////////////////////////////////////////////////////////////////////////
double scoreAv8[] = { 2.54, 2.51, 2.72, 3.01, 3.31, 3.61, 3.90, 4.19, 4.47,
		4.74, 4.99, 5.22, 5.46, 5.70, 5.94, 6.13, 6.36, 6.52, 6.68, 6.91 };
double scoreSd8[] = { 1.33, 0.88, 0.73, 0.71, 0.74, 0.80, 0.86, 0.92, 0.98,
		1.04, 1.08, 1.10, 1.15, 1.19, 1.23, 1.25, 1.32, 1.34, 1.36, 1.45 };
		1.25e-28, 4.08e-29, 1.31e-29, 4.18e-30, 1.32e-30, 4.12e-31, 1.27e-31,
		3.90e-32, 1.18e-32, 3.55e-33, 1.06e-33, 3.11e-34, 9.06e-35, 2.61e-35,
		7.47e-36, 2.11e-36, 5.91e-37, 1.64e-37, 4.50e-38, 1.22e-38, 3.29e-39,
		8.77e-40, 2.31e-40, 6.05e-41, 1.56e-41, 4.00e-42, 1.02e-42, 2.55e-43,
		6.33e-44, 1.56e-44, 3.80e-45, 9.16e-46, 2.19e-46, 5.17e-47, 1.21e-47,
		2.81e-48, 6.45e-49, 1.46e-49, 3.30e-50 };
double tablePtoZ[] = { 0.00, 0.73, 1.24, 1.64, 1.99, 2.30, 2.58, 2.83, 3.07,
		3.29, 3.50, 3.70, 3.89, 4.07, 4.25, 4.42, 4.58, 4.74, 4.89, 5.04, 5.19,
		5.33, 5.46, 5.60, 5.73, 5.86, 5.99, 6.11, 6.23, 6.35, 6.47, 6.58, 6.70,
		6.81, 6.92, 7.02, 7.13, 7.24, 7.34, 7.44, 7.54, 7.64, 7.74, 7.84, 7.93,
		8.03, 8.12, 8.21, 8.30, 8.40, 8.49, 8.57, 8.66, 8.75, 8.84, 8.92, 9.01,
		9.09, 9.17, 9.25, 9.34, 9.42, 9.50, 9.58, 9.66, 9.73, 9.81, 9.89, 9.97,
		10.04, 10.12, 10.19, 10.27, 10.34, 10.41, 10.49, 10.56, 10.63, 10.70,
		10.77, 10.84, 10.91, 10.98, 11.05, 11.12, 11.19, 11.26, 11.32, 11.39,
		11.46, 11.52, 11.59, 11.66, 11.72, 11.79, 11.85, 11.91, 11.98, 12.04,
		12.10, 12.17, 12.23, 12.29, 12.35, 12.42, 12.48, 12.54, 12.60, 12.66,
		12.72, 12.78, 12.84, 12.90, 12.96, 13.02, 13.07, 13.13, 13.19, 13.25,
		13.31, 13.36, 13.42, 13.48, 13.53, 13.59, 13.65, 13.70, 13.76, 13.81,
		13.87, 13.92, 13.98, 14.03, 14.09, 14.14, 14.19, 14.25, 14.30, 14.35,
		14.41, 14.46, 14.51, 14.57, 14.62, 14.67, 14.72, 14.77, 14.83, 14.88,
		14.93 };
///////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////
double ce_a1(char *name1, char *name2, XYZ *ca1, XYZ *ca2, int nse1, int nse2,
		int *align_se1, int *align_se2, int& lcmp, double rmsdThrJoin,
		int isPrint) {
	/*
	 int distAll=0;
	 int winSize_=8;
...
double zStrAlign(int4 *align_se, int lcmp, XYZ *ca1, XYZ *ca2) {
	...
	for (int l1 = 0; l1 < lcmp; l1++)
		if (align_se[l1] != -1 && align_se[l1 + lcmp] != -1) {
			if (ca1[align_se[l1]].X < 1e10
					&& ca2[align_se[l1 + lcmp]].X < 1e10) {
				lali++;
			}
		} else {
			lgap++;
				...
				for (int l2 = l1 + 1; l2 < lcmp; l2++)
					if (align_se[l2] != -1 && align_se[l2 + lcmp] != -1)
						if (abs(align_se[l1] - align_se[l2]) > 1)
							if (ca1[align_se[l2]].X < 1e10
									&& ca2[align_se[l2 + lcmp]].X < 1e10) {
								d +=
										fabs(
												ca1[align_se[l1]].dist(
														ca1[align_se[l2]])
														- ca2[align_se[l1 + lcmp]].dist(
																ca2[align_se[l2
																		+ lcmp]]));
								nd++;
							}
