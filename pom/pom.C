...
////////////////////////////////////////////////////////////////////
template<class A> void InsertItems(A **array, int4 n, A *vals, int4 nVals,
		int4 *nObj, int4 diff, int cacheSize = DEFAULT_CACHE) {
...
////////////////////////////////////////////////////////////////////
AminoacidProperty properties[] = { "A", "ALA", 1, 71.09, 15.9, 25.9, 39.2, 23.1,
		37.4, 1.33, 0.79, "V", "VAL", 2, 99.15, 47.7, 8.6, 38.5, 49.6, 19.6,
		0.93, 1.50, "L", "LEU", 3, 113.17, 63.6, 0.0, 38.6, 57.6, 10.1, 1.20,
		1.09, "I", "ILE", 4, 113.17, 63.6, 0.0, 39.2, 83.6, 7.5, 1.02, 1.44,
		"C", "CYS", 5, 103.15, 28.0, 7.4, 26.3, 40.3, 7.4, 0.98, 1.11, "M",
		"MET", 6, 131.21, 62.8, 4.9, 35.7, 44.3, 3.9, 1.25, 0.99, "P", "PRO", 7,
		97.13, 41.0, 21.0, 40.2, 73.5, 66.2, 0.55, 0.73, "F", "PHE", 8, 147.19,
		77.2, 1.2, 38.6, 76.1, 5.5, 1.00, 1.20, "Y", "TYR", 9, 163.19, 78.5,
		9.9, 34.4, 70.8, 30.1, 0.75, 1.44, "W", "TRP", 10, 186.23, 100.0, 4.9,
		37.7, 100.0, 13.8, 1.03, 1.17, "D", "ASP", 11, 115.10, 31.3, 100.0, 0.0,
		17.5, 45.0, 1.04, 0.68, "N", "ASN", 12, 114.12, 35.4, 63.0, 31.3, 2.4,
		46.1, 0.96, 0.80, "E", "GLU", 13, 129.13, 47.2, 93.8, 3.2, 17.8, 48.6,
		1.41, 0.59, "Q", "GLN", 14, 128.15, 51.3, 45.7, 34.4, 0.0, 43.6, 1.07,
		0.91, "H", "HIS", 15, 137.16, 49.2, 43.2, 59.2, 23.1, 28.1, 1.18, 0.94,
		"S", "SER", 16, 87.09, 18.1, 32.1, 34.8, 1.9, 40.5, 0.81, 1.00, "T",
		"THR", 17, 101.12, 34.0, 21.0, 45.7, 1.9, 35.3, 0.81, 1.15, "R", "ARG",
		18, 156.20, 70.8, 51.9, 100.0, 22.6, 50.1, 0.97, 1.08, "K", "LYS", 19,
		128.19, 68.0, 64.2, 86.9, 43.5, 54.3, 1.15, 0.81, "G", "GLY", 20, 57.07,
		0.0, 37.0, 38.5, 2.7, 54.0, 0.67, 0.96 };
///////////////////////////////////////////////////////////////////////////
int aa_decode[25] = { 1, 0, 5, 11, 13, 8, 20, 15, 4, 0, 19, 3, 6, 12, 0, 7, 14,
		18, 16, 17, 0, 2, 10, 0, 9 };
char aa_code[21] = { 'A', 'V', 'L', 'I', 'C', 'M', 'P', 'F', 'Y', 'W', 'D', 'N',
		'E', 'Q', 'H', 'S', 'T', 'R', 'K', 'G', 'X' };
///////////////////////////////////////////////////////////////////////////
DB::DB(char *path_) {
...
void Property::saveHeader(int with_load_mode) {
	...
	if (!size)
		db->writeFile(ref, nItems + 1, adrRef_, 4, fd);
}
////////////////////////////////////////////////////////////////////
...
void Property::extendItem(int4 index, int1* item, int4 nValues) {
	...
	int i;
	for (i = 0; i < nValues_; i++)
		_item[i] = item_[i];
	for (i = 0; i < nValues; i++)
		_item[nValues_ + i] = item[i];
...
void Property::addItem(int1 item) {
	...
	if (!size)
		AddToArray(&ref, nProp, nItems + 1);
	nItems++;

	if (load_in_memory)
		AddToArray(&prop1, item, nProp);
...
void Property::addItem(int1* item, int4 nValues) {
	...
	if (load_in_memory)
		AddToArrayN(&prop1, item, nProp, nValues);
...
void Property::addItem(int2* item, int4 nValues) {
	...
	if (load_in_memory)
		AddToArrayN(&prop2, item, nProp, nValues);
...
void Property::addItem(int4* item, int4 nValues) {
	...
	if (load_in_memory)
		AddToArrayN(&prop4, item, nProp, nValues);
...
void Property::addItem(flt4* item, int4 nValues) {
	...
	if (load_in_memory)
		AddToArrayN(&propf, item, nProp, nValues);
	db->writeFile(item, nValues, adrRef, 4, fd);
	nProp += nValues;
	if (!size)
		ref[nItems] = nProp;
...
void Property::addItem(flt8* item, int4 nValues) {
	...
	if (load_in_memory)
		AddToArrayN(&propd, item, nProp, nValues);
...
int1* Property::item1(int4 index, int copy_array) {
	...
	if (!load_in_memory)
		db->readFile(tmp, l_tmp, adrProp + i_tmp, 1, fd);
...
int2* Property::item2(int4 index, int copy_array) {
	...
	if (!load_in_memory)
		db->readFile(tmp, l_tmp, adrProp + i_tmp * 2, 2, fd);
...
int4* Property::item4(int4 index, int copy_array) {
	...
	if (!load_in_memory)
		db->readFile(tmp, l_tmp, adrProp + i_tmp * 4, 4, fd);
...
flt8* Property::itemd(int4 index, int copy_array) {
	...
	if (!load_in_memory)
		db->readFile(tmp, l_tmp, adrProp + i_tmp * 8, 8, fd);
...
int4 Property::find(int4 t1, int4 t2, int& next) {
	if (size != 1 || (type != D_INT1 && type != D_INT2 && type != D_INT4)
			|| !load_in_memory) {
		printf(
				"Property::find - '%s' access error size=%d type=%d load_in_memory=%d\n",
	...
	if (type == D_INT1) {
		for (; iObj < nItems; iObj++) {
			if (t2 == -1) {
				...
				if (prop1[iObj] >= t1 && prop1[iObj] <= t2)
					return (iObj);
			}
		}
		return (-1);
	...
	if (type == D_INT2) {
		for (; iObj < nItems; iObj++) {
			if (t2 == -1) {
				...
				if (prop2[iObj] >= t1 && prop2[iObj] <= t2)
					return (iObj);
			}
		}
		return (-1);
	...
	if (type == D_INT4) {
		for (; iObj < nItems; iObj++) {
			if (t2 == -1) {
				...
				if (prop4[iObj] >= t1 && prop4[iObj] <= t2)
					return (iObj);
			}
		}
		return (-1);
...
int4 Property::find(flt4 t1, flt4 t2, int& next) {
	...
	for (; iObj < nItems; iObj++) {
		if (t2 == -1) {
			...
			if (propf[iObj] >= t1 && propf[iObj] <= t2)
				return (iObj);
		}
	}
	return (-1);
...
#ifndef INT2_OFF
		if (type == D_INT2)
			DeleteArray(&coll2, nClasses);
#endif
		if (type == D_INT4)
			DeleteArray(&coll4, nClasses);
	} else {
#ifndef INT2_OFF
		if (coll2t)
			delete[] coll2t;
...
int2 Collection::itemValue2(int4 index, int4 cindex) {
	if (load_in_memory) {
		...
		if (index_ != index) {
			if (coll2t)
				delete[] coll2t;
			db->readFile(coll2, nColl[index], adrColl + nCollt[index] * 2, 2,
					fd);
			index_ = index;
		}
...
int4 Collection::itemValue4(int4 index, int4 cindex) {
	if (load_in_memory) {
		...
		if (index_ != index) {
			if (coll4t)
				delete[] coll4t;
			db->readFile(coll4, nColl[index], adrColl + nCollt[index] * 4, 4,
					fd);
			index_ = index;
		}
...
#ifndef INT2_OFF
	if (type == D_INT2)
		return ((int) itemValue2(index, cindex));
#endif
	if (type == D_INT4)
		return ((int) itemValue4(index, cindex));
	return 0;
}
////////////////////////////////////////////////////////////////////
#ifndef INT2_OFF
	...
	if (load_in_memory) {
		...
		if (index_ != index) {
			...
			if (nColl[index]) {
				...
				db->readFile(coll2t, nColl[index], adrColl + nCollt[index] * 2,
						2, fd);
				index_ = index;
			}
		}
	...
	if (load_in_memory) {
		...
		if (index_ != index) {
			...
			if (nColl[index]) {
				...
				db->readFile(coll4t, nColl[index], adrColl + nCollt[index] * 4,
						4, fd);
				index_ = index;
			}
		}
...
int2 Monomers::nAtom(int iM) {
	if (!monNatom)
		monNatom = new Property("n_atom.mon", db);
	return (*monNatom->item2(iM));
}
//////////////////////////////////////////////////////////////////
char* Monomers::atoms(int iM) {
	if (!monAtom)
		monAtom = new Property("atom.mon", db);
...
int Monomers::type(int iM) {
	if (!monType)
		monType = new Property("type.mon", db);
...
int Monomers::prev(int iM) {
	if (!monPrev)
		monPrev = new Property("prev.mon", db);
	return (*monPrev->item2(iM));
}
//////////////////////////////////////////////////////////////////
int Monomers::next(int iM) {
	if (!monNext)
		monNext = new Property("next.mon", db);
...
Entities::~Entities() {
	if (nEnc) {
		...
		for (int i = 0; i < nEnc; i++)
			if (encSEnum[i])
				DeleteArray(&encSEnum[i], encNSE[i]);
...
void Entities::addCom(int4 iCom, int addMode) {
	int err, i, j, k, iView;
	int2 nEncAdd, nEnpAdd, iXYZ, nAtom;
	char *comEnt_;
	int4 nEncOld;
	int4 iEnc;
	...
	Property n_enc_com("n_enc.com", db);
	...
	ExtendArrayValue(&encpMonType, nEncOld, nEnc, (int1*) NULL);
	 ...
	 Property i_enc_com("i_enc.com", db);
	 Property i_enp_enc("i_enp.enc", db);
	int b_enc_com = *i_enc_com.item4(iCom);

	 Property name_enc("name.enc", 0, db);
	 Property n_se_enc("n_se.enc", 0, db);
...
void Entities::addEnc(int4 iEnc, int isAdd) {
	...
	ExtendArrayValue(&encpMonType, nEncOld, nEnc, (int1*) NULL);
	 ...
	 Property i_com_enc("i_com.enc", db);
	 Property i_enp_enc("i_enp.enc", db);
...
void Entities::addEnp(int4 iEnp, int isAdd) {
	...
	int4 nEncOld;
	 ...
	 Property i_enc_enp("i_enc.enp", db);
...
void Entities::load_encSEnum(int iE) {
	if (!encSEnum[iE]) {
		 Property prop("sen_pdb.enc", 0, db);
...
void Entities::load_encpBfac(int iE) {
	if (!encpBfac[iE] && enpNum[iE] >= 0) {
		 Property prop("bfac_flt.enp", 0, db);
...
void Entities::load_monType(int iE) {
	if (!encpMonType[iE] && enpNum[iE] >= 0) {
		 Property prop("se_type.enp", 0, db);
...
void Entities::reread() {
	for (int i = 0; i < nEnc; i++)
		addEnc(encNum[i], i ? 1 : 0);
}
//////////////////////////////////////////////////////////////////
char* IEntity::atoms(int is) {
	if (!entities->encSE[iE])
		entities->load_encSE(iE);
	return (entities->monomers->atoms(*(entities->encSE[iE] + is)));
}
//////////////////////////////////////////////////////////////////
char* IEntity::atom(int is, int ia) {
	if (!entities->enc_se_xyz[iE])
		entities->load_enc_se_xyz(iE);
	if (!entities->encSE[iE])
		entities->load_encSE(iE);
...
ISubentity ISubentity::neighbor(int shift) {
	int iS_ = iS + shift;
	ISubentity se = *this;
	if (iS_ < 0 || iS_ >= entities->encNSE[iE])
		iS_ = -1;
	se.iS = iS_;
	iM = -1;
	return (se);
...
char* IAtom::name() {
	...
	strncpy(_name,
			entities->monomers->atom(*(entities->encSE[iE] + iS_),
...
IMonomer::IMonomer(ISubentity isubentity) {
	...
	iM =
			*(isubentity.entities->encSE[isubentity.myEntityIndex()]
					+ isubentity());
	_name[4] = '\0';
}
//////////////////////////////////////////////////////////////////
char* IMonomer::atomName(IAtom atom) {
	...
	int iS_ =
			atom.mySubentityIndex() >= 0 ?
					atom.mySubentityIndex() :
					*(atom.entities->enc_xyz_se[atom.myEntityIndex()] + atom());
	strncpy(_name,
			monomers->atom(iM,
					atom()
							- *(atom.entities->enc_se_xyz[atom.myEntityIndex()]
...
IBond::IBond(IAtom atom) {
	if (!atom.entities->enc_xyz_se[atom.iE])
		atom.entities->load_enc_xyz_se(atom.iE);
	if (!atom.entities->encSE[atom.iE])
		atom.entities->load_encSE(atom.iE);
	monomers = atom.entities->monomers;
	int iS_ =
			atom.mySubentityIndex() >= 0 ?
					atom.mySubentityIndex() :
					*(atom.entities->enc_xyz_se[atom.myEntityIndex()] + atom());
	iM = *(atom.entities->encSE[atom.myEntityIndex()] + atom.mySubentityIndex())
			- 1;
	iB = 0;
}
//////////////////////////////////////////////////////////////////
