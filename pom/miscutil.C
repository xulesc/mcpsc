...
void andArrays(int *array1, int nA1, int *array2, int nA2, int **result,
	...
	for (int n = 0; n < nA1; n++) {
		while (array1[n] > array2[n2] && n2 < nA2)
			n2++;
...
int sortedArrayContains(int *array, int number, int nArray) {
	if (array[nArray - 1] < number)
		return 0;

	int high = nArray - 1, low = 0, mid;

	while (low < high && (low + 1) != high) {
		if (number == array[high] || number == array[low])
			return 1;
...
int file_cmp(char *file_test, char *file_tmpl) {
	...
	if (!star)
		return (strcmp(file_test, file_tmpl));
	if (strncmp(file_test, file_tmpl, strlen(file_tmpl) - strlen(star)))
		return (1);
...
void AddBuffer(char *** array, char * buffer, int Narray) {
