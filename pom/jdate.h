 ...
 */
/* $Revision: 2.0 $ $Date: 1999/02/11 02:42:05 $ */
class Weekdays;
class Months;
class JDate;
...
public:
	...
	// Two functions to convert a Julian day (jday) into a
	// normal date (jdate)

	static int jday(int, int, int); // (month, day, year)
	static int jday(char *, int, int); // ("month", day, year)

	// and vice-versa
	static int *jdate(int);
};
