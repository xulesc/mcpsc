...
PDBVersionType Derive::getPDBVersionType(int index) {
	...
	if (*obs.item4(index) == 0 && *spr.item4(index) == 0)
		return CURRENT;
	if (*obs.item4(index) != 0)
		return OBSLTE;
	if (*obs.item4(index) == 0 && *spr.item4(index) != 0)
		return SPRSDE;
	return UNK;
...
void Derive::matchType(PDBVersionType vType, int *ids, int nIds, int **result,
		int *nResult) {
