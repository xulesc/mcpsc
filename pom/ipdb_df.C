...
int DF::assignDF(int nCom1, int nCom2, int log_mode) {
	return assignDF(DBPATH, nCom1, nCom2, 1, NUM_DF, log_mode);
}
///////////////////////////////////////////////////////////////////////////
int DF::assignDF(char *db, int nCom1, int nCom2, int log_mode) {
	return assignDF(db, nCom1, nCom2, 1, NUM_DF, log_mode);
}
///////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////
int DF::assignDF(char *db_path, int nCom1, int nCom2, int df1, int df2,
		int log_mode) {

	if (!log_mode)
		printf("calculating DF %d to %d for compounds %d to %d\n", df1, df2,
				nCom1, nCom2);
...
void DF::bfac(int nCom1, int nCom2, int log_mode, int df2) {
	...
	if (!P_BFAC_FLT_ENP && !P_C_A_ENP && !P_N_ENP_COM)
		return;
	...
	if (P_BFAC_FLT_ENP)
		bfac_flt_enp.open("bfac_flt.enp", 0);
	if (P_C_A_ENP)
		c_a_enp.open("c_a.enp", 0);
	if (P_N_ENP_COM)
	...
	for (int iC = nCom1; iC < nCom2; iC++) {
		...
		if (nEnt > 0) {
			...
			for (IEntity iE1(&entities1); iEnt < nEnt; ++iE1, iEnt++) {
				...
				tmf_no: if (P_BFAC_FLT_ENP)
					bfac_flt_enp.addItem(prop[iEnt], iE1.NSE());
				if (P_C_A_ENP)
					c_a_enp.addItem(ca[iEnt], iE1.NSE() * 3);
			}
...
void DF::obsNspr(int nCom1, int nCom2, int log_mode, int df2) {
	...
	if (P_OBS_IDS_COM)
		obs_ids_com.open("obs_ids.com", 1);
	if (P_SPR_IDS_COM)
		spr_ids_com.open("spr_ids.com", 1);

	int4 *justOne = new int4[1];
	justOne[0] = -1;
	...
	if (P_OBS_DAT_COM)
		obsdat.open("obs_dat.com", 1);
	if (P_SPR_DAT_COM)
		sprdat.open("spr_dat.com", 1);
	...
	for (iC = 0; iC < spr_com.nItems; iC++) {
		...
		if (nObsIdIdx[iC])
			obsIdIdx[iC] = obs_com.item4(iC, 1);
	}
	...
	for (iC = 0; iC < spr_com.nItems; iC++) {
		...
		for (i = 0; i < nIds; i++) {
			...
			if (nSupers == 1 && supers[0] == -1 && !*obsdat.item4(index[i])) {
	...
	if (P_CURRENT_COM)
		current_com.open("current.com", 1);
	current_com.clear();
	for (iC = 0; iC < spr_com.nItems; iC++)
...
void DF::collect(int nCom1, int nCom2, int log_mode, int df2) {
	...
	for (iC = 0; iC < nCom2; iC++) {
		...
		if (res >= lRes && res <= hRes && *res_com.itemf(obsIds[0]) >= lRes
				&& *res_com.itemf(obsIds[0]) <= hRes) {

			index = (int) (*res_com.itemf(obsIds[0]) * 1000 - res * 1000) / 100
					+ nCRes / 2;
		...
		if (res > *res_com.itemf(obsIds[0]))
			misc_col.add(39, iC);
		else if (res < *res_com.itemf(obsIds[0]))
			misc_col.add(40, iC);
		else
			misc_col.add(38, iC);
		...
		if (nChains >= lCha && nChains <= hCha
			...
			index = *n_enp_com.item2(obsIds[0]) - nChains + hCha - lCha;
...
char *DF::flt2str(float *f) {
	...
	sprintf(out, "%.2f %.2f %.2f %.2f %.2f %.2f", f[0], f[1], f[2], f[3], f[4],
			f[5]);
	// printf("%s\n", out);
	return out;
}
...
void DF::calcChi1(int nCom1, int nCom2, int log_mode, int df2) {
	...
	if (!nCom1) {
		if (P_CHI1_ENP)
			chi1_enp.create("chi1.enp", D_FLT4, 6, 0);
		if (P_CHI1_COM)
			chi1_com.create("chi1.com", D_FLT4, 1, 0);
	} else {
		if (P_CHI1_ENP)
			chi1_enp.open("chi1.enp", 0);
		if (P_CHI1_COM)
			chi1_com.open("chi1.com", 0);
	}
	...
	for (int iCom = nCom1; iCom < nCom2; iCom++) {
		...
		for (IEntity iE(&ents); iEnt < nEnt; ++iE, ++iEnt, entIn++) {
			...
			array[6] += (float) (array[1] * statsSum[8].num
					+ array[3] * statsSum[9].num + array[5] * statsSum[10].num);

			total += statsSum[8].num + statsSum[9].num + statsSum[10].num;

			if (P_CHI1_ENP)
				chi1_enp.addItem(array);
			delete[] statsSum;
		}
		...
		if (P_CHI1_COM)
			chi1_com.addItem(array[6]);
	}

	delete[] array;
}

void DF::proteinFeatures(char *db, int nCom1, int nCom2, int log_mode,
		int df2) {

	if (!MOOSE_PATTERNS_GROUP)
		return;

	char *prefTab[] = { "exp", "pol", "bfac", "spol", "shyd", "svol", "sexp",
			"siso" };

	char *nameTab[] = { "Exposure", "Polarity", "Temperature factor",
			"Static Polarity", "Static Hydrophobicity", "Static Volume",
			"Static Exposure", "Static Isoelectric Point" };

	int iProp, iPT0 = 0, i, j, k, l, seq_, nStates = 6, iMin, iMax, fSize, ind,
			iS1, iS2, aa_code_, nSum, nnHits1 = 240, nnPatterns2 = (int4) pow(
					nStates, 5), nnHits2 = 6 * nnPatterns2;
	...
	int4 tEnt1, tEnt2, *aa_freq, *aa_freq8, *pDistr0_8, nDistr0_8,
			sePropAvI[160], nsePropAv, nStates8;
	int4 nProp;
	...
	double *states = new double[nStates], *sePropAvD = new double[160], clSize,
			pMin, pMax, pSpan, dProp, sum;
...
//	  printf("%d entities found\n", tEnt);
	...
	for (int iPT = iPT0; iPT < 8; iPT++) {
		...
		if (nCom1)
			...
			imm.create(file_buf, D_INT2, nnHits1);
		...
		if (nCom1)
			...
			ip6.create(file_buf, D_INT2, nnHits2);
...
//	      fprintf(tmp, "<H2>%s sites in proteins</H2>\n", nameTab[iPT]);
		for (k = 0; k < 2; k++) {
			...
			fprintf(tmp, "<HR> <P><H3>Property values %s threshold</H3>\n",
					k == 0 ? "below" : "above");
			...
			for (int p = 0; p < 2; p++) {
				...
				for (i = p * 10; i < p * 10 + 10; i++) {
					fprintf(tmp, "<TH> %d </TH>", (int) (i * clSize + clSize));
				}
...
//		  pp8.clear();
		...
		for (i = 0; i < 20; i++)
			sePropAvC[iPT * 20 + i] = (int1) (
					sePropAvI[iPT * 20 + i] > 0 ?
							((sePropAvD[iPT * 20 + i] / sePropAvI[iPT * 20 + i]
									- pMin) / pSpan * 254 + 1) :
							0);
...
void DF::mooseReport(int nCom1, int nCom2, int log_mode, int df2) {
	...
	for (iCom = nCom1; iCom < nCom2; iCom++) {
		...
		for (iEnp = *i_enp_com.item4(iCom); iEnp < *i_enp_com.item4(iCom + 1);
				iEnp++) {
			entMolWeight = 0;
			entType = ' ';
			ssAlpha = 0;
...
void DF::status_com(int log_mode) {
	...
	Property id_com("id.com"), file_com("file.com"), status_com("status.com"),
			obs_dat_com("obs_dat.com");
	...
	for (int ic = 0; ic < nCom; ic++) {
		...
		if (!DB::testPathFile(cpath, cfile))
			stat |= 0x1;

		if (*obs_dat_com.item4(ic) != 0)
			stat |= 0x2;
