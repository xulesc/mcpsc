...
// 
//////////////////////////////////////////////////////////////////
template<class A> void AddToArray2(A ** array, A value1, A value2, int Narray) {
	A * array_;
...
;
//////////////////////////////////////////////////////////////////
template<class A> void AddToArrayMULT(A ** array, double * values, int Narray,
		int Nvalues, int mult) {
	A * array_;
	int i;
	...
	for (i = 0; i < Narray; i++)
		array_[i] = *(*array + i);
	for (i = 0; i < Nvalues; i++)
		array_[Narray + i] = (A) (values[i] * mult);
	if (Narray)
		delete[] (*array);
	(*array) = array_;
}
;
///////////////////////////////////////////////////////////////////////////
template<class A> void InsertItems(A **array, int n, A *vals, int nVals,
		int *nObj, int diff) {

	(*nObj) += diff;
	A *newArray = new A[(*nObj)];
	int i, old, nw = 0;

	for (old = 0; old < n; old++, nw++)
		newArray[nw] = *(*array + old);

	for (i = 0; i < nVals; i++, nw++)
		newArray[nw] = vals[i];
...
;
////////////////////////////////////////////////////////////////////
template<class A> void AddToArray(A ** array, A value, int Narray,
...
///////////////////////////////////////////////////////////////////////////
#ifndef INT2_OFF
template<class A> void AddToArray(A ** array, A value, int2 Narray,
		int cacheSize = DEFAULT_CACHE) {
...
#endif
///////////////////////////////////////////////////////////////////////////
template<class A> void AddToArrayN(A ** array, A * values, int Narray,
		int Nvalues, int cacheSize = DEFAULT_CACHE) {
	if (Nvalues <= 0)
		return;
...
//////////////////////////////////////////////////////////////////
#ifndef INT2_OFF
template<class A> void AddToArrayN(A ** array, A * values, int2 Narray,
		int2 Nvalues, int cacheSize = DEFAULT_CACHE) {
	if (Nvalues <= 0)
		return;
...
#endif
//////////////////////////////////////////////////////////////////
template<class A> void ExtendArray(A ** array, int NarrayOld, int NarrayNew) {
	A * array_;
	array_ = new A[NarrayNew];
	for (int i = 0; i < NarrayOld; i++)
		array_[i] = *(*array + i);
	if (NarrayOld)
		delete[] (*array);
	(*array) = array_;
...
;
//////////////////////////////////////////////////////////////////
template<class A> void ExtendArrayValue(A ** array, int NarrayOld,
		int NarrayNew, A value) {
	int i;
	A * array_;
	array_ = new A[NarrayNew];
	for (i = 0; i < NarrayOld; i++)
		array_[i] = *(*array + i);
	for (i = NarrayOld; i < NarrayNew; i++)
		array_[i] = value;
	if (NarrayOld)
		delete[] (*array);
	(*array) = array_;
...
int Monomer::checkBond(flt4 * xyzI, flt4 * xyzJ) {
	double dx, dy, dz;
	if ((dx = fabs((double) *xyzI - (double) *xyzJ)) > bondDist)
		return (0);
	if ((dy = fabs((double) *(xyzI + 1) - (double) *(xyzJ + 1))) > bondDist)
		return (0);
	if ((dz = fabs((double) *(xyzI + 2) - (double) *(xyzJ + 2))) > bondDist)
		return (0);
	if (dx * dx + dy * dy + dz * dz > bondDist2)
		return (0);
	return (1);
}
//////////////////////////////////////////////////////////////////
			...
			if (bond[i] == iAtom) {
				...
				for (j = k + 2; j < nBond * 2; j++) {
					if (bond[j] == jAtom) {
						...
						if (iAtom_ == jAtom_) {
							Dk = dist2(xyz + iAtom * 3 - 3,
									xyz + jAtom * 3 - 3);
							Di = dist2(xyz + iAtom * 3 - 3,
									xyz + iAtom_ * 3 - 3);
							Dj = dist2(xyz + jAtom * 3 - 3,
									xyz + jAtom_ * 3 - 3);
							if (Dk > Di && Dk > Dj) {
								removeBond(k / 2);
							} else {
...
int Monomer::findAtom(char * atom) {
	for (int i = 0; i < nAtom; i++)
		if (!strncmp(atomNames + i * 4, atom, 4))
			return (i);
	return (-1);
}
//////////////////////////////////////////////////////////////////
...
Molecule::Molecule(Connectivity * con_, Tlpdb * tlpdb_) {
	...
	if (P_N_XYZ_ENC)
		n_xyz_enc.open("n_xyz.enc", 0);
	if (P_XYZ_ENC)
		xyz_enc.open("xyz.enc", 0);
	if (P_SE_XYZ_ENC)
		se_xyz_enc.open("se_xyz.enc", 0);
	if (P_XYZ_SE_ENC)
		xyz_se_enc.open("xyz_se.enc", 0);

	if (P_NAME_ENC)
		name_enc.open("name.enc", 0);
	if (P_NAME_ENP)
		name_enp.open("name.enp", 0);
	if (P_N_SE_ENC)
		n_se_enc.open("n_se.enc", 0);
	if (P_N_SE_ENP)
		n_se_enp.open("n_se.enp", 0);
	if (P_SE_ENC)
		se_enc.open("se.enc", 0);
	if (P_N_ENP_COM)
		n_enp_com.open("n_enp.com", 0);
	if (P_N_ENC_COM)
		n_enc_com.open("n_enc.com", 0);
	if (P_I_ENP_COM)
		i_enp_com.open("i_enp.com", 0);
	if (P_I_ENC_COM)
		i_enc_com.open("i_enc.com", 0);

	if (P_I_COM_ENC)
		i_com_enc.open("i_com.enc", 0);
	if (P_I_COM_ENP)
		i_com_enp.open("i_com.enp", 0);

	if (P_I_ENC_ENP)
		i_enc_enp.open("i_enc.enp", 0);
	if (P_I_ENP_ENC)
		i_enp_enc.open("i_enp.enc", 0);

#ifndef MINIMAL
	if (P_SEN_PDB_ENC)
		sen_pdb_enc.open("sen_pdb.enc", 0);
#endif

	if (P_BFAC_ENC)
		bfac_enc.open("bfac.enc", 0);

#ifndef MINIMAL
	if (P_EXP_ENP)
		exp_enp.open("exp.enp", 0);
	if (P_POL_ENP)
		pol_enp.open("pol.enp", 0);
#endif

	if (P_K_S_ENP)
		k_s_enp.open("k_s.enp", 0);
	if (P_SEQ_ENP)
		seq_enp.open("seq.enp", 0);

	if (P_RES_COM)
		res_com.open("res.com", 0);

#ifndef MINIMAL
	if (P_EXPDTA_COM)
		expdta_com.open("expdta.com", 0);
	if (P_EXPDTA_TXT_COM)
		expdta_txt_com.open("expdta_txt.com", 0);
	if (P_EXP_FLT_ENP)
		exp_flt_enp.open("exp_flt.enp", 0);
	if (P_POL_FLT_ENP)
		pol_flt_enp.open("pol_flt.enp", 0);
	if (P_SEQ_FLT_ENP)
		seq_flt_enp.open("seq_flt.enp", 0);

	if (P_ISEQ_FLT_ENP)
		iseq_flt_enp.open("iseq_flt.enp", 0);
	if (P_ISEQ_ENP)
		iseq_enp.open("iseq.enp", 0);
#endif

	if (P_SE_TYPE_ENP)
		se_type_enp.open("se_type.enp", 0);

	if (P_RELDAT_COM)
		rel_dat_com.open("reldat.com", 0);
	if (P_CURRENT_COM)
		current_com.open("current.com", 0);

	if (P_OBS_DAT_COM)
		obs_date_com.open("obs_dat.com", 0);
	if (P_OBS_IDS_COM)
		obs_ids_com.open("obs_ids.com", 0);
	if (P_OBS_COM)
		obs_com.open("obs.com", 0);
	if (P_SPR_COM)
		spr_date_com.open("spr_dat.com", 0);
	if (P_SPR_IDS_COM)
		spr_ids_com.open("spr_ids.com", 0);
	if (P_SPR_COM)
		spr_com.open("spr.com", 0);

	if (P_UNITCELL_COM)
		unit_cell_com.open("unitcell.com", 0);
	if (P_SPCGRP_COM)
		space_grp_com.open("spcgrp.com", 0);
	if (P_ZVAL_COM)
		z_val_com.open("zval.com", 0);
	if (P_RVAL_COM)
		rval_com.open("rval.com", 0);

	if (P_ID_COM)
		id_com.open("id.com", 0);
	if (P_FILE_COM)
		file_com.open("file.com", 0);
	if (P_DATE_TEX_COM)
		date_tex_com.open("date_tex.com", 0);
	if (P_DATE_INT_COM)
		date_int_com.open("date_int.com", 0);

#ifndef MINIMAL
	if (P_TITLE_COM)
		title_com.open("title.com", 0);
	if (P_COMPND_COM)
		compnd_com.open("compnd.com", 0);
	if (P_SOURCE_COM)
		source_com.open("source.com", 0);
	if (P_DATE_TEX_COM)
		date_tex_com.open("date_tex.com", 0);
	if (P_DATE_INT_COM)
		date_int_com.open("date_int.com", 0);
	if (P_HEADER_COM)
		header_com.open("header.com", 0);
	if (P_AUTH_COM)
		auth_com.open("auth.com", 0);
	if (P_JRNL_COM)
		jrnl_com.open("jrnl.com", 0);
	if (P_SITE_COM)
		site_com.open("site.com", 0);
	if (P_SSBOND_COM)
		ssbond_com.open("ssbond.com", 0);
	if (P_NDBMAP_COM)
		ndbmap_com.open("ndbmap.com", 0);
#endif
}
//////////////////////////////////////////////////////////////////
int Molecule::makeMolecule(char * fileName) {
	...
	if (obsCount)
		DeleteArray(&obsIds, obsCount);
	obsCount = 0;
	if (sprCount)
		DeleteArray(&sprIds, sprCount);
	sprCount = 0;
	...
	if (tlpdb->cboxes[5] && P_K_S_ENP)
		assignKS();

#ifndef MINIMAL
	if (tlpdb->cboxes[6]
			&& (P_EXP_ENP || P_EXP_FLT_ENP || P_POL_ENP || P_POL_FLT_ENP
					|| P_EXP_1B_ENP || P_POL_1B_ENP || P_EXP_IP6 || P_POL_IP6))
		assignEnv();
#endif

	if (P_SEQ_FLT_ENP || P_ISEQ_ENP || P_ISEQ_FLT_ENP || P_EXP_FLT_ENP
			|| P_POL_FLT_ENP || P_SE_TYPE_ENP)
		assignFiltProp();
...
void Molecule::writeDB() {
	...
	if (P_SPCGRP_COM)
		space_grp_com.addItem(spcgrp);

	if (P_RELDAT_COM)
		rel_dat_com.addItem(relDate);

	if (P_ZVAL_COM)
		z_val_com.addItem(Zval);
	Zval = -1;
	if (P_RVAL_COM)
		rval_com.addItem(rvalue);
	rvalue = 0.;

	if (P_CURRENT_COM)
		current_com.addItem((int4) -1);

	if (P_OBS_COM)
		obs_com.addItem((int4) -1);
	if (P_OBS_DAT_COM)
		obs_date_com.addItem(obsDate);
	if (obsCount) {
		char *obsIdsTmp;
		StringArrayToBuffer(obsCount, &obsIdsTmp, &obsIds);
		if (P_OBS_IDS_COM)
			obs_ids_com.addItem(obsIdsTmp);
		delete[] obsIdsTmp;
	} else {
		if (P_OBS_IDS_COM)
			obs_ids_com.addItem("\0");
	}

	if (P_SPR_COM)
		spr_com.addItem((int4) -1);
	if (P_SPR_DAT_COM)
		spr_date_com.addItem(sprDate);
	if (sprCount) {
		char *sprIdsTmp;
		StringArrayToBuffer(sprCount, &sprIdsTmp, &sprIds);
		if (P_SPR_IDS_COM)
			spr_ids_com.addItem(sprIdsTmp);
		delete[] sprIdsTmp;
	} else {
		if (P_SPR_IDS_COM)
			spr_ids_com.addItem("\0");
	}

	for (i = 0; i < nChain; i++)
		if (P_N_XYZ_ENC)
			n_xyz_enc.addItem(comNXYZ[i]);
	for (i = 0; i < nChain; i++)
		if (P_XYZ_ENC)
			xyz_enc.addItem(comXYZ[i], comNXYZ[i] * 3);

	if (P_ID_COM)
		id_com.addItem(id);
	if (P_FILE_COM)
		file_com.addItem(file);

	if (P_DATE_TEX_COM)
		date_tex_com.addItem(date);
	if (P_DATE_INT_COM)
		date_int_com.addItem(dateInt);

#ifndef MINIMAL
	compressText(&title);
	if (P_TITLE_COM)
		title_com.addItem(title);

	compressText(&compnd);
	if (P_COMPND_COM)
		compnd_com.addItem(compnd);

	compressText(&source);
	if (P_SOURCE_COM)
		source_com.addItem(source);

	if (P_HEADER_COM)
		header_com.addItem(header);

	compressText(&author);
	if (P_AUTH_COM)
		auth_com.addItem(author);

	compressText(&jrnl);
	if (P_JRNL_COM)
		jrnl_com.addItem(jrnl);

	if (P_SITE_COM)
		site_com.addItem(site);
	if (P_SSBOND_COM)
		ssbond_com.addItem(ssBond);

	compressText(&ndbMap);
	if (P_NDBMAP_COM)
		ndbmap_com.addItem(ndbMap);
#endif
	...
	for (i = 0; i < nChain; i++)
		if (P_N_SE_ENC)
			n_se_enc.addItem(nMon[i]);
	for (i = 0; i < nChain; i++)
		if (P_SE_ENC)
			se_enc.addItem(mon[i], nMon[i]);
	for (i = 0; i < nChain; i++)
		if (P_I_COM_ENC)
			i_com_enc.addItem(nCom);

	for (i = 0; i < nChain; i++)
		if (P_I_ENP_ENC)
			i_enp_enc.addItem((int4) (i < nPolyChain ? nEnp + i : -1));
	/*
	 for(i=0; i<nChain; i++) {
	 if(nMon[i]>1) {
	 nPolyChain=i+1;
	...
	for (i = 0; i < nPolyChain; i++)
		if (P_N_SE_ENP)
			n_se_enp.addItem(nMon[i]);

	for (i = 0; i < nPolyChain; i++)
		if (P_I_COM_ENP)
			i_com_enp.addItem(nCom);

	for (i = 0; i < nPolyChain; i++)
		if (P_I_ENC_ENP)
			i_enc_enp.addItem((int4) (nEnc + i));

	if (P_I_ENP_COM)
		i_enp_com.addItem(n_se_enp.getObjectSize());
	if (P_I_ENC_COM)
		i_enc_com.addItem(n_se_enc.getObjectSize());

	if (P_N_ENC_COM)
		n_enc_com.addItem(nChain);
	if (P_N_ENP_COM)
		n_enp_com.addItem(nPolyChain);

	for (i = 0; i < nChain; i++) {
		if (P_BFAC_ENC)
			bfac_enc.addItem(comTmf[i], comNXYZ[i]);
	}

#ifndef MINIMAL
	for (i = 0; i < nPolyChain; i++)
		if (P_EXP_ENP)
			exp_enp.addItem(exp[i], nMon[i]);
	for (i = 0; i < nPolyChain; i++)
		if (P_POL_ENP)
			pol_enp.addItem(pol[i], nMon[i]);
#endif

	for (i = 0; i < nPolyChain; i++)
		if (P_K_S_ENP)
			k_s_enp.addItem(ks[i]);
	...
	for (i = 0; i < nChain; i++) {
		...
		for (j = 0; j < i; j++)
			if (!strncmp(comEnt[j], comEnt[i], entNamLen))
				nSameEnt++;

		if (nSameEnt)
			sprintf(comEnt[i], "%s:%d", comEnt[i], nSameEnt);

		if (P_NAME_ENC)
			name_enc.addItem(comEnt[i]);

		if (i < nPolyChain) {
			if (P_NAME_ENP)
				name_enp.addItem(comEnt[i]);
		...
		if (P_SE_XYZ_ENC)
			se_xyz_enc.addItem(comSExyz, nMon[i] + 1);
		if (P_XYZ_SE_ENC)
			xyz_se_enc.addItem(comXYZse, comNXYZ[i]);
	...
	if (P_RES_COM)
		res_com.addItem(resolution);

#ifndef MINIMAL
	if (P_EXPDTA_COM)
		expdta_com.addItem(expdta);
	if (P_EXPDTA_TXT_COM) {
		compressText(&expdta_txt);
		if (!expdta_txt)
			expdta_txt_com.addItem("X-RAY DIFFRACTION");
		else
			expdta_txt_com.addItem(expdta_txt);
	}
#endif
...
void Molecule::addMonomer(Monomer * M) {
	...
	AddToArrayN(&comXYZ[nChain - 1], M->xyz, comNXYZ[nChain - 1] * 3,
			M->nAtom * 3);
	AddToArrayN(&comTmf[nChain - 1], M->tmf, comNXYZ[nChain - 1],
			(int) M->nAtom);
	nMon[nChain - 1]++;
	comNXYZ[nChain - 1] += M->nAtom;
}
//////////////////////////////////////////////////////////////////
char *expdtaValues[] = { "ELECTRON", "FIBER DI", "FLUORESC", "NEUTRON", "NMR",
		"NMR SOLI", "NMR SOLU", "SYNCHROT", "THEORETI", "X-RAY DI", NULL };
//////////////////////////////////////////////////////////////////
Monomer * Molecule::getMonomer() {
	...
	int monomerStatus = 0;
	char *rem3 = NULL;
	read_1: if (modelStatus == 2)
		return (NULL);
	if (!bufferStatus)
		if (!fgets(buffer, 100, pdbFile)) {
			return (M);
		} else {
			if (buffer[strlen(buffer) - 1] < 32)
	...
	if (!strncmp("HEADER", buffer, 6))
		addHeader(buffer);

#ifndef MINIMAL
	if (!strncmp("COMPND", buffer, 6))
		addCompnd(buffer);
	if (!strncmp("SOURCE", buffer, 6))
		addSource(buffer);
	if (!strncmp("AUTHOR", buffer, 6))
		addAuthor(buffer);
	if (!strncmp("JRNL  ", buffer, 6))
		addJrnl(buffer);
#endif

	if (!strncmp("SEQRES", buffer, 6))
		addSeqres(buffer);
	if (!strncmp("CRYST1", buffer, 6))
		addCryst(buffer);
	if (!strncmp("MODEL ", buffer, 6))
		chainStatus = 1;

	if (!strncmp("OBSLTE", buffer, 6))
		addObslte(buffer);
	if (!strncmp("SPRSDE", buffer, 6))
		addSprsde(buffer);
	if (!strncmp("REVDAT   1", buffer, 10))
		addRelDate(buffer);

	if (!strncmp("SSBOND", buffer, 6)) {
		buffer[16] = buffer[21] = buffer[30] = buffer[35] = '\0';
		if (buffer[15] != ' ')
			addText(&ssBond, &buffer[15]);
		sprintf(tmp, "%d", atoi(&buffer[17]));
		addText(&ssBond, tmp);
		addText(&ssBond, ":");
		if (buffer[29] != ' ')
			addText(&ssBond, &buffer[29]);
	...
	if (!strncmp("SITE", buffer, 4)) {
		...
		buffer[14] = buffer[17] = buffer[21] = buffer[32] = buffer[43] =
				buffer[54] = buffer[61] = '\0';
	...
	if (!strncmp("EXPDTA", buffer, 6)) {
		...
		for (int i = 0; expdtaValues[i]; i++)
			if (!strncmp(expdtaValues[i], &buffer[10],
					strlen(expdtaValues[i]))) {
				expdta = i;
				break;
			}
	}
	 ...
	 */
	if (!strncmp("ATOM  ", buffer, 6) || !strncmp("HETATM", buffer, 6)) {
		if (strlen(buffer) < 55)
			goto read_1;
		if (buffer[16] != ' ' && buffer[16] != 'A' && buffer[16] != '1')
			goto read_1;
		if (buffer[13] == 'H')
			goto read_1;
		if (buffer[13] == 'Q')
			goto read_1;
		if (!strncmp("EXC", &buffer[17], 3))
			goto read_1;
		if (!monomerStatus) {
			M = new Monomer;
			...
			M->pdbChain != buffer[21] || strncmp(M->pdbNum, &buffer[22], 5))
				goto exit_1;
		}
		buffer[66] = '\0';
		tmf_ = atof(&buffer[60]);
		buffer[54] = '\0';
		xyz_[2] = atof(&buffer[46]);
		buffer[46] = '\0';
		xyz_[1] = atof(&buffer[38]);
		buffer[38] = '\0';
		xyz_[0] = atof(&buffer[30]);
		atomNames_[4] = '\0';
		if (buffer[12] == '\0')
			buffer[12] = ' ';
		if (buffer[13] == '\0')
...
void Molecule::addSeqres(char * buffer) {
	...
	if (!seqNchain)
		addChainStatus = 1;
	if (seqNchain)
		if (seqChainCode[seqNchain - 1] != buffer[11])
			addChainStatus = 1;
	if (addChainStatus) {
...
void Molecule::matchAtomSeqres() {
	int i, j, k1, k2, *seqMatch, matchIndex, matchStatus, matchChain,
			*chainStatus, nChain_, firstChain;
	int2 index;
	...
	for (i = 0; i < seqNchain; i++) {
		...
		nMon_ = 0;
		NXYZ_ = 0;
		for (j = 0; j < seqL[i] + 2; j++)
			seqMatch[j] = -1;
		for (j = 0; j < nChain; j++) {
			if (chainStatus[j])
				continue;
			if (seqChainCode[i] != chainCode[j])
				continue;
			...
			if (matchStatus == 0 && nMon[j] > 10
				...
				for (k2 = 0; k2 < nMon[j] && k2 < 10; k2++)
					if (strcmp(*(seq[i] + k2),
							con->monCode3.item1(*(mon[j] + 1 + k2))))
						goto break_n_term;
				seqMatch[matchStatus] = j;
				matchStatus++;
				nTermHet = 1;
			}
			break_n_term:
			...
			for (k1 = matchIndex; k1 < seqL[i] && k1 + nMon[j] <= seqL[i] + 1;
					k1++) {
...
int Molecule::assignKS() {
	int iC, i, j;
	int NSE, k, strandStatus, breakStatus;
	char *seq, *turn3, *turn4, *turn5, *pbridge, *abridge, *bend, *helix3,
			*helix4, *helix5, *summary;
	int monCA1, monCA2, monCA3, CA1, CA2, CA3, aCA1, aCA2, aCA3;
	XYZ ca1, ca2, ca3;
	//FILE * fks=fopen("ks","a");
	...
	for (iC = 0; iC < nChain; iC++) {
		...
		helix5 = new char[NSE];
		summary = new char[NSE];
		if (!seq || !turn3 || !turn4 || !turn5 || !pbridge || !abridge || !bend
				|| !helix3 || !helix4 || !helix5 || !summary)
			return (1);
		for (NSE = 0; NSE < nMon[iC]; NSE++) {
			seq[NSE] = *con->monCode1.item1(*(mon[iC] + NSE));
		...
		for (i = 1; i < NSE; i++) {
			...
			if (i < NSE - 4) {
				...
				if (CA1 >= 0 && CA2 >= 0 && CA3 >= 0) {
					...
					ca3.Z = *(comXYZ[iC] + CA3 * 3 + 2);
					if (ca1.dist(ca2) + ca2.dist(ca3) != 0.0)
						if (ca1.dist(ca3) / (ca1.dist(ca2) + ca2.dist(ca3))
								< 0.82)
							bend[i + 2] = 'S';
				}
			}
			...
			if (summary[i] == 'x' || summary[i] == 'X') {
				if (i > 0)
					if (summary[i - 1] == 'E')
						strandStatus = 1;
				if (i < NSE - 1)
					if (summary[i + 1] == 'x' || summary[i + 1] == 'X')
						strandStatus = 1;
				summary[i] = strandStatus ? 'E' : 'B';
			}
		}
...
#define NN_ARRAY_SIZE 1000
	...
	char atomType;
	XYZ a1, a2;
	flt4 aX, aY, aZ, bX, bY, bZ, wX, wY, wZ, *waterX = new flt4[nWaterData],
			*waterY = new flt4[nWaterData], *waterZ = new flt4[nWaterData];
	double dX, dY, dZ;
	flt4 rAtom = 1.8, rWater = 1.4;
	flt4 rCont = (rAtom + rWater) * 2.0, rWaterCont = rAtom + rWater;
	double dCont2 = ((double) rCont) * ((double) rCont), dWaterCont2 =
			((double) rWaterCont) * ((double) rWaterCont);
	long nW, nP;
			...
			for (i = i1; i < i2; i++) {
				...
				for (jSE = 0; jSE < NSE; jSE++) {
					...
					for (j = j1; j < j2; j++) {
						...
						nnArray[nn] = j;
						nnTypes[nn] =
								(atomType == 'N' || atomType == 'O') ? 1 : 0;
						nnRange[nn] =
								(jSE == iSE
										|| (abs(jSE - iSE) == 1 && j - j1 < 4)) ?
										0 : 1;
						nn++;
					}
				}
				...
				for (iW = 0; iW < nWaterData; iW++) {
					...
					for (j_ = 0; j_ < nn; j_++) {
						j = nnArray[j_];
						bX = *(comXYZ[iE] + j * 3);
						if (fabs(wX - bX) > rWaterCont)
							continue;
						bY = *(comXYZ[iE] + j * 3 + 1);
						if (fabs(wY - bY) > rWaterCont)
							continue;
						bZ = *(comXYZ[iE] + j * 3 + 2);
						if (fabs(wZ - bZ) > rWaterCont)
							continue;
						dX = wX - bX;
						dY = wY - bY;
						dZ = wZ - bZ;
						if (dX * dX + dY * dY + dZ * dZ > dWaterCont2)
							continue;
						goto cont1;
					}
					nW++;
					continue;
					cont1: if (nnTypes[j_] && nnRange[j_])
						nP++;
				}
			}
			*(*(exp + iE) + iSE) = (
					i2 > i1 ?
							((double) nW) / ((double) ((i2 - i1) * nWaterData))
									* 100.0 :
							-1);
			*(*(pol + iE) + iSE) = (
					i2 > i1 ?
							((double) nP) / ((double) ((i2 - i1) * nWaterData))
									* 100.0 :
							-1);
		}
	}
...
//  Property se_type_enp("se_type.enp", 0, db);
	...
	for (iE1 = 0; iE1 < nEnt; ++iE1) {
		...
		prop0[iE1] = new char[nMon[iE1] + 1];
		*(prop0[iE1] + nMon[iE1]) = '\0';
		prop3[iE1] = new char[nMon[iE1] + 1];
		*(prop3[iE1] + nMon[iE1]) = '\0';
		...
		for (int i = 0; i < nMon[iE1]; i++) {
			...
			*(prop_isf[iE1] + i) =
					*con->monNatom.item2(monomer) < 4 ? 20 : aa_code_;
			*(prop1[iE1] + i) =
					(*(prop0[iE1] + i) == 'X') ? -1.0 : *(exp[iE1] + i);
			*(prop2[iE1] + i) =
					(*(prop0[iE1] + i) == 'X') ? -1.0 : *(pol[iE1] + i);
		...
		if (P_SEQ_FLT_ENP)
			seq_flt_enp.addItem(prop0[iE1]);
		if (P_ISEQ_ENP)
			iseq_enp.addItem(prop_is[iE1], nMon[iE1]);
		if (P_ISEQ_FLT_ENP)
			iseq_flt_enp.addItem(prop_isf[iE1], nMon[iE1]);
		if (P_EXP_FLT_ENP)
			exp_flt_enp.addItem(prop1[iE1], nMon[iE1]);
		if (P_POL_FLT_ENP)
			pol_flt_enp.addItem(prop2[iE1], nMon[iE1]);

#endif

		if (P_SE_TYPE_ENP)
			se_type_enp.addItem(se_type[iE1], nMon[iE1]);
	}
...
///////////////////////////////////////////////////////////////////////////
int Molecule::HBondingEnergy(int iC, int iS1, char * atom1, int iS2,
		char * atom2) {
	int iMon1 = *(mon[iC] + iS1), iMon2 = *(mon[iC] + iS2), iMonP, C, O, N, CA,
			CN, iA1 = *(monXYZ[iC] + iS1), iA2 = *(monXYZ[iC] + iS2), iAP;
	if (!strncasecmp(atom1, "O", 1)) {
		if ((O = findAtom(iMon1, " O  ")) < 0)
			return (0);
		if ((N = findAtom(iMon2, " N  ")) < 0)
			return (0);
		if (fabs(*(comXYZ[iC] + O * 3) - *(comXYZ[iC] + N * 3))
				> distanceCutoff)
			return (0);
		if (fabs(*(comXYZ[iC] + O * 3 + 1) - *(comXYZ[iC] + N * 3 + 1))
				> distanceCutoff)
			return (0);
		if (fabs(*(comXYZ[iC] + O * 3 + 2) - *(comXYZ[iC] + N * 3 + 2))
				> distanceCutoff)
			return (0);
		if ((C = findAtom(iMon1, " C  ")) < 0)
			return (0);
		if ((CA = findAtom(iMon2, " CA ")) < 0)
			return (0);
		if (iS2 < 1)
			return (0);
		iMonP = *(mon[iC] + iS2 - 1);
		iAP = *(monXYZ[iC] + iS2 - 1);
		if ((CN = findAtom(iMonP, " C  ")) < 0)
			return (0);
		C += iA1;
		O += iA1;
		N += iA2;
		...
		if ((O = findAtom(iMon2, " O  ")) < 0)
			return (0);
		if ((N = findAtom(iMon1, " N  ")) < 0)
			return (0);
		if (fabs(*(comXYZ[iC] + O * 3) - *(comXYZ[iC] + N * 3))
				> distanceCutoff)
			return (0);
		...
		if (fabs(*(comXYZ[iC] + O * 3 + 2) - *(comXYZ[iC] + N * 3 + 2))
				> distanceCutoff)
			return (0);
		if ((C = findAtom(iMon2, " C  ")) < 0)
			return (0);
		if ((CA = findAtom(iMon1, " CA ")) < 0)
			return (0);
		if (iS1 < 1)
			return (0);
		iMonP = *(mon[iC] + iS1 - 1);
	...
	n.Z = *(comXYZ[iC] + N * 3 + 2);
	ca.X = *(comXYZ[iC] + CA * 3);
	ca.Y = *(comXYZ[iC] + CA * 3 + 1);
	ca.Z = *(comXYZ[iC] + CA * 3 + 2);
	cn.X = *(comXYZ[iC] + CN * 3);
	cn.Y = *(comXYZ[iC] + CN * 3 + 1);
	...
	double e = 0.42 * 0.20 * 332.0
			* (1.0 / o.dist(n) + 1.0 / c.dist(h) - 1.0 / o.dist(h)
					- 1.0 / c.dist(n));
//  printf("\n");
//  printf("C: %.2f, %.2f, %.2f\n", C->d_x(), C->d_y(), C->d_z());
//  printf("O: %.2f, %.2f, %.2f\n", O->d_x(), O->d_y(), O->d_z());
...
int Molecule::findAtom(int iMon, char * atom) {
	...
	for (int i = 0; i < nAtom_tmp; i++)
		if (!strncmp(atom, atom_tmp + i * 4, 4))
			return (i);
	return (-1);
}
//////////////////////////////////////////////////////////////////
...
int Connectivity::read() {
	int i, err;
	int2 dummyI, monPrevNext[2];
	char * dummyC;

	if (P_CODE3_MON)
		monCode3.open("code3.mon");
	if (P_CODE1_MON)
		monCode1.open("code1.mon");
	if (P_N_BOND_MON)
		monNbond.open("n_bond.mon");
	if (P_BOND_MON)
		monBond.open("bond.mon");
	if (P_N_ATOM_MON)
		monNatom.open("n_atom.mon");
	if (P_ATOM_MON)
		monAtom.open("atom.mon");
	if (P_PREV_MON)
		monPrev.open("prev.mon");
	if (P_NEXT_MON)
		monNext.open("next.mon");
	if (P_TYPE_MON)
		monType.open("type.mon");
...
int Connectivity::search(Monomer *M) {
	...
	while ((i = monCode3.find(M->name, 0, pos, next_search)) != -1) {
		next_search = 1;
		if (M->nAtom != *monNatom.item2(i))
			continue;
		if (M->nBond != *monNbond.item2(i))
			continue;
		if (M->prevA - 1 != *monPrev.item2(i)
				|| M->nextA - 1 != *monNext.item2(i))
			continue;
		atomNames_ = monAtom.item1(i);
		if (M->nAtom)
			if (strcmp(M->atomNames, atomNames_))
				goto break_1;
		monBond_ = monBond.item2(i);
		for (j = 0; j < *monNbond.item2(i) * 2; j++)
			if (M->bond[j] != monBond_[j])
				goto break_1;
		return (i);
		break_1: ;
	}
	...
	if (P_CODE3_MON)
		monCode3.addItem(M->name);
	if (P_N_ATOM_MON)
		monNatom.addItem(M->nAtom);
	if (P_N_BOND_MON)
		monNbond.addItem(M->nBond);
	if (P_ATOM_MON)
		monAtom.addItem(M->atomNames);
	if (P_BOND_MON)
		monBond.addItem(M->bond, (M->nBond * 2));

	monCode3_ = M->name;

	monCode1_ = 'X';
	monType_ = 1;
	if (!strcmp(monCode3_, "ALA"))
		monCode1_ = 'A';
	if (!strcmp(monCode3_, "VAL"))
		monCode1_ = 'V';
	if (!strcmp(monCode3_, "LEU"))
		monCode1_ = 'L';
	if (!strcmp(monCode3_, "ILE"))
		monCode1_ = 'I';
	if (!strcmp(monCode3_, "CYS"))
		monCode1_ = 'C';
	if (!strcmp(monCode3_, "MET"))
		monCode1_ = 'M';
	if (!strcmp(monCode3_, "PRO"))
		monCode1_ = 'P';
	if (!strcmp(monCode3_, "PHE"))
		monCode1_ = 'F';
	if (!strcmp(monCode3_, "TYR"))
		monCode1_ = 'Y';
	if (!strcmp(monCode3_, "TRP"))
		monCode1_ = 'W';
	if (!strcmp(monCode3_, "ASP"))
		monCode1_ = 'D';
	if (!strcmp(monCode3_, "ASN"))
		monCode1_ = 'N';
	if (!strcmp(monCode3_, "GLU"))
		monCode1_ = 'E';
	if (!strcmp(monCode3_, "GLN"))
		monCode1_ = 'Q';
	if (!strcmp(monCode3_, "HIS"))
		monCode1_ = 'H';
	if (!strcmp(monCode3_, "SER"))
		monCode1_ = 'S';
	if (!strcmp(monCode3_, "THR"))
		monCode1_ = 'T';
	if (!strcmp(monCode3_, "ARG"))
		monCode1_ = 'R';
	if (!strcmp(monCode3_, "LYS"))
		monCode1_ = 'K';
	if (!strcmp(monCode3_, "GLY"))
		monCode1_ = 'G';
	...
	if (monCode1_ == 'X' && M->findAtom(" CA ") == -1)
		monType_ = 0;

	if (P_CODE1_MON)
		monCode1.addItem((int1) monCode1_);
	if (P_TYPE_MON)
		monType.addItem((int1) monType_);

	if (P_PREV_MON)
		monPrev.addItem((int2) (M->prevA - 1));
	if (P_NEXT_MON)
		monNext.addItem((int2) (M->nextA - 1));
 ...
 */
void alt_entry(int argc, char **argv) {
	Tlpdb tlpdb;
	if (!strcmp(argv[1], "update"))
		tlpdb.update(argc, argv);
	if (!strcmp(argv[1], "scratch"))
		tlpdb.scratch(argc, argv);
}
...
void Tlpdb::update(int argc, char **argv) {
	...
	if (nLoadList) {
		...
		char command[100];
		sprintf(command, "rm -rf %s", argv[4]);
		system(command);
		sprintf(command, "cp -pfr %s %s", argv[2], argv[4]);
		system(command);
		...
		sprintf(command, "mv %s %s.bak", argv[2], argv[2]);
		system(command);
		sprintf(command, "ln -s %s %s", argv[4], argv[2]);
		system(command);
		sprintf(command, "cp -pf %s/* %s.bak", argv[4], argv[2]);
		system(command);
		sprintf(command, "rm -f %s", argv[2]);
		system(command);
		sprintf(command, "mv %s.bak %s", argv[2], argv[2]);
		system(command);
...
void Tlpdb::scratch(int argc, char **argv) {
	...
	AddBuffer(&loadList, argv[3], nLoadList);
	nLoadList++;
...
void Tlpdb::dialog() {
	...
	if (mode == 's') {
		...
		if (isObs) {
			strcpy(path, OBSDIR);
			printf("obsolete directory? [%s]\t", path);
			getInput(&path);

			strcpy(files, "pdb*.ent");
			printf("obsolete frame? [%s]\t", files);
			getInput(&files);
		}
	...
	if (mode == 'l') {
		...
		getlist: printf("File with list? [%s]\t", list1);
		getInput(&list1);
...
void Tlpdb::scan() {
	...
	if (cboxes[4]) { // check with db
		...
		if (searchResult) {
			...
			fprintf(stderr, "Could not find id.com in %s - no db checking!\n",
					db_path);
	}
	...
	if (isObs)
		total += findFiles(path, files, 0);
	if (isCur)
		total += findFiles(currentPath, currFiles, 0);
	...
	if (isObs)
		findFiles(path, files, 1);
	if (isCur)
		findFiles(currentPath, currFiles, 1);

	printf("%d entries selected\n", nLoadList);

	int j, jMin, cmpCode, pos = strlen(loadList[0]) - 7;
	char *tmp;
	...
	if (sort_flag) {
		...
		for (i = 0; i < nLoadList - 1; i++) {
			...
			for (j = i; j < nLoadList; j++) {
				...
				if (cmpCode == 0)
					cmpCode = strcmp(loadList[jMin] + pos - 1,
							loadList[j] + pos - 1);
				if (cmpCode > 0)
					jMin = j;
			}
...
int Tlpdb::findFiles(char *path_, char *searchString, int mode) {
	...
	while ((d = readdir(handleDir))) {
		if (file_cmp(d->d_name, searchString))
			continue;

		if (cboxes[2]) { // check with second list
			for (i = 0; i < nCheckList; i++)
				for (j = 0; j < strlen(d->d_name) - strlen(checkList[i]) + 1;
						j++)
					if (!strncasecmp(checkList[i], d->d_name + j,
							strlen(checkList[i])))
						goto checkOK;
			continue;
		}

		checkOK: if (cboxes[4]) { // check with db
...
int Tlpdb::makeLoadListF() {
	...
	while (fscanf(loadListF, "%s", buffer) != EOF) {
		AddBuffer(&loadList, buffer, nLoadList);
		nLoadList++;
	}
...
//           load|size|type|prop/coll|on/off
		"code3.mon", 1, 4, D_INT1, 0, P_CODE3_MON, "code1.mon", 1, 1, D_INT1, 0,
		P_CODE1_MON, "n_bond.mon", 1, 1, D_INT2, 0, P_N_BOND_MON, "bond.mon", 1,
		0, D_INT2, 0, P_BOND_MON, "n_atom.mon", 1, 1, D_INT2, 0, P_N_ATOM_MON,
		"atom.mon", 1, 0, D_INT1, 0, P_ATOM_MON, "prev.mon", 1, 1, D_INT2, 0,
		P_PREV_MON, "next.mon", 1, 1, D_INT2, 0, P_NEXT_MON, "type.mon", 1, 1,
		D_INT1, 0, P_TYPE_MON,

		"id.com", 1, 5, D_INT1, 0, P_ID_COM, "file.com", 1, 0, D_INT1, 0,
		P_FILE_COM, "status.com", 1, 1, D_INT1, 0, P_STATUS_COM,

#ifndef MINIMAL
		"title.com", 0, 0, D_INT1, 0, P_TITLE_COM, "compnd.com", 0, 0, D_INT1,
		0, P_COMPND_COM, "source.com", 0, 0, D_INT1, 0, P_SOURCE_COM,
#endif

		"date_tex.com", 0, 0, D_INT1, 0, P_DATE_TEX_COM, "date_int.com", 1, 1,
		D_INT4, 0, P_DATE_INT_COM,

#ifndef MINIMAL
		"header.com", 0, 0, D_INT1, 0, P_HEADER_COM, "auth.com", 0, 0, D_INT1,
		0, P_AUTH_COM, "jrnl.com", 0, 0, D_INT1, 0, P_JRNL_COM, "expdta.com", 1,
		1, D_INT1, 0, P_EXPDTA_COM, "expdta_txt.com", 1, 0, D_INT1, 0,
		P_EXPDTA_TXT_COM, "ec.com", 0, 0, D_INT1, 0, P_EC_COM, "ssbond.com", 1,
		0, D_INT1, 0, P_SSBOND_COM, "site.com", 0, 0, D_INT1, 0, P_SITE_COM,
		"ndbmap.com", 1, 0, D_INT1, 0, P_NDBMAP_COM,
#endif

		"res.com", 1, 1, D_FLT4, 0, P_RES_COM, "n_enp.com", 1, 1, D_INT2, 0,
		P_N_ENP_COM, "i_enp.com", 1, 1, D_INT4, 0, P_I_ENP_COM, "n_enc.com", 1,
		1, D_INT2, 0, P_N_ENC_COM, "i_enc.com", 1, 1, D_INT4, 0, P_I_ENC_COM,

		"reldat.com", 1, 1, D_INT4, 0, P_RELDAT_COM, "obs_ids.com", 1, 0,
		D_INT1, 0, P_OBS_IDS_COM, "obs.com", 1, 0, D_INT4, 0, P_OBS_COM,
		"current.com", 1, 0, D_INT4, 0, P_CURRENT_COM, "obs_dat.com", 1, 1,
		D_INT4, 0, P_OBS_DAT_COM, "spr_ids.com", 1, 0, D_INT1, 0, P_SPR_IDS_COM,
		"spr.com", 1, 0, D_INT4, 0, P_SPR_COM, "spr_dat.com", 1, 1, D_INT4, 0,
		P_SPR_DAT_COM,

		"unitcell.com", 0, 6, D_FLT4, 0, P_UNITCELL_COM, "spcgrp.com", 0, 0,
		D_INT1, 0, P_SPCGRP_COM, "zval.com", 1, 1, D_INT2, 0, P_ZVAL_COM,
		"rval.com", 1, 1, D_FLT4, 0, P_RVAL_COM,

		"name.enc", 1, 0, D_INT1, 0, P_NAME_ENC, "i_com.enc", 1, 1, D_INT4, 0,
		P_I_COM_ENC, "i_enp.enc", 1, 1, D_INT4, 0, P_I_ENP_ENC, "n_se.enc", 0,
		1, D_INT2, 0, P_N_SE_ENC, "se.enc", 0, 0, D_INT2, 0, P_SE_ENC,

#ifndef MINIMAL
		"sen_pdb.enc", 0, 0, D_INT1, 0, P_SEN_PDB_ENC,
#endif

		"n_xyz.enc", 0, 1, D_INT4, 0, P_N_XYZ_ENC, "xyz.enc", 0, 0, D_FLT4, 0,
		P_XYZ_ENC, "bfac.enc", 0, 0, D_FLT4, 0, P_BFAC_ENC, "se_xyz.enc", 0, 0,
		D_INT4, 0, P_SE_XYZ_ENC, "xyz_se.enc", 0, 0, D_INT4, 0, P_XYZ_SE_ENC,

		"name.enp", 1, 0, D_INT1, 0, P_NAME_ENP, "i_com.enp", 1, 1, D_INT4, 0,
		P_I_COM_ENP, "i_enc.enp", 1, 1, D_INT4, 0, P_I_ENC_ENP, "type.enp", 1,
		1, D_INT1, 0, P_TYPE_ENP,
...
#ifndef MINIMAL
		"alpha_c.enp", 1, 1, D_FLT4, 0, P_ALPHA_C_ENP, "beta_c.enp", 1, 1,
		D_FLT4, 0, P_BETA_C_ENP, "alpha_n.enp", 1, 1, D_INT2, 0, P_ALPHA_N_ENP,
		"beta_n.enp", 1, 1, D_INT2, 0, P_BETA_N_ENP, "ss_seg.enp", 0, 0, D_INT1,
		0, P_SS_SEG_ENP,
#endif
...
#ifndef MINIMAL
		"seq_flt.enp", 1, 0, D_INT1, 0, P_SEQ_FLT_ENP, "iseq.enp", 1, 0, D_INT1,
		0, P_ISEQ_ENP, "iseq_flt.enp", 1, 0, D_INT1, 0, P_ISEQ_FLT_ENP,
		"c_a.enp", 0, 0, D_FLT4, 0, P_C_A_ENP,
#endif

		"k_s.enp", 1, 0, D_INT1, 0, P_K_S_ENP, "fds.enp", 0, 1, D_FLT4, 0,
		P_FDS_ENP,

		"chi1.enp", 0, 6, D_FLT4, 0, P_CHI1_ENP, "chi1.com", 1, 1, D_FLT4, 0,
		P_CHI1_COM, "fds.com", 1, 1, D_FLT4, 0, P_FDS_COM,

		"se_type.enp", 0, 0, D_INT1, 0, P_SE_TYPE_ENP,

#ifndef MINIMAL
		"exp.enp", 1, 0, D_FLT4, 0, P_EXP_ENP, "exp_flt.enp", 1, 0, D_FLT4, 0,
		P_EXP_FLT_ENP, "pol.enp", 1, 0, D_FLT4, 0, P_POL_ENP, "pol_flt.enp", 1,
		0, D_FLT4, 0, P_POL_FLT_ENP, "bfac_flt.enp", 1, 0, D_FLT4, 0,
		P_BFAC_FLT_ENP,

		"exp_1b.enp", 1, 0, D_INT1, 0, P_EXP_1B_ENP, "pol_1b.enp", 1, 0, D_INT1,
		0, P_POL_1B_ENP, "bfac_1b.enp", 1, 0, D_INT1, 0, P_BFAC_1B_ENP,
		"sexp_1b.enp", 1, 0, D_INT1, 0, P_SEXP_1B_ENP, "spol_1b.enp", 1, 0,
		D_INT1, 0, P_SPOL_1B_ENP, "shyd_1b.enp", 1, 0, D_INT1, 0, P_SHYD_1B_ENP,
		"svol_1b.enp", 1, 0, D_INT1, 0, P_SVOL_1B_ENP, "siso_1b.enp", 1, 0,
		D_INT1, 0, P_SISO_1B_ENP,

		"exp.pp6", 1, 120, D_INT4, 0, P_EXP_PP6, "pol.pp6", 1, 120, D_INT4, 0,
		P_POL_PP6, "bfac.pp6", 1, 120, D_INT4, 0, P_BFAC_PP6, "exp.pp8", 1, 160,
		D_INT4, 0, P_EXP_PP8, "pol.pp8", 1, 160, D_INT4, 0, P_POL_PP8,
		"bfac.pp8", 1, 160, D_INT4, 0, P_BFAC_PP8,

		"ks.pp3", 1, 60, D_INT4, 0, P_KS_PP3, "prop8.ave", 1, 160, D_INT1, 0,
		P_PROP8_AVE, "prop8.sts", 1, 56, D_FLT4, 0, P_PROP8_STS,
#endif
...
void Tlpdb::createProperties() {
	...
	i_enp_com.addItem((int4) 0);
	Property i_enc_com("i_enc.com");
	i_enc_com.addItem((int4) 0);
}
//////////////////////////////////////////////////////////////////
