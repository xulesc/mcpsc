...
#ifndef INT2_OFF
enum DataType {
	D_INT1, D_INT2, D_INT4, D_FLT4, D_FLT8
};
#else
enum DataType {D_INT1, D_INT2=2, D_INT4=2, D_FLT4, D_FLT8};
#endif
enum CopyArray {
	COPY_ARRAY_OFF, COPY_ARRAY_ON
};
enum LoadInMemory {
	LOAD_IN_MEMORY_OFF, LOAD_IN_MEMORY_ON
};
///////////////////////////////////////////////////////////////////////////
class IAtom;
class IEntity;
...
public:
	...
	~Property();
	void create(char *name_, DataType type_, int4 size_, int load_in_memory_ =
			LOAD_IN_MEMORY_ON, DB *db_ = db_default, int cacheSize =
			DEFAULT_CACHE);
	void clear();
	...
	int4 find(int4 t1, int4 t2 = -1) {
		int next = 0;
		return (find(t1, t2, next));
	}
	;
	int4 find(flt4 t1, flt4 t2, int& next);
	int4 find(flt4 t1, flt4 t2 = -1) {
		int next = 0;
		return (find(t1, t2, next));
	}
	;
	int4 find(flt8 t1, flt8 t2, int& next);
	int4 find(flt8 t1, flt8 t2 = -1) {
		int next = 0;
		return (find(t1, t2, next));
	}
	;
...
public:
	...
	~Collection();
	void create(char *name_, DataType type_, int4 nClasses_, DB *db_ =
			db_default);
	void clear();
	void open(char *name_, DB *db_ = db_default);
	...
	;
	int4 getCollectionSize() {
		return (nClasses);
	}
	;
	int4 getClassSize(int4 index) {
		return (nColl[index]);
	}
	;
	int4* getClassSize() {
...
public:
	...
	;
	IEntity(Entities *entities_) {
		entities = entities_;
		iE = 0;
	}
	;
	operator void*() {
		return ((void*) (iE >= 0 && iE < entities->nEnc));
	}
	;
	void operator++() {
		if (iE >= 0)
	...
	;
	int1* name() {
		return (entities->encName[iE]);
	}
	;
	int1* atoms(int is);
	int1* atom(int is, int ia);
	int NSE() {
		return (entities->encNSE[iE]);
	}
	;
	void set(int iE_) {
		iE = iE_;
...
public:
	...
	;
	operator void*() {
		return ((void*) (iS >= 0 && iS < entities->encNSE[iE]));
	}
	;
	void operator++() {
		if (iS >= 0) {
	...
	// entity info
	int1* pdbNum() {
		if (!entities->encSEnum[iE])
			entities->load_encSEnum(iE);
		return (*(entities->encSEnum[iE] + iS));
	}
	;
	int1 ks() {
		if (entities->enpNum[iE] < 0)
			return (' ');
		if (!entities->encpKS[iE])
			entities->load_encpKS(iE);
	...
	flt4 exp() {
		if (entities->enpNum[iE] < 0)
			return (-1.0);
		if (!entities->encpExp[iE])
			entities->load_encpExp(iE);
	...
	flt4 pol() {
		if (entities->enpNum[iE] < 0)
			return (-1.0);
		if (!entities->encpPol[iE])
			entities->load_encpPol(iE);
	...
	int monType() {
		if (entities->enpNum[iE] < 0)
			return (0);
		if (!entities->encpMonType[iE])
			entities->load_monType(iE);
	...
	flt4 bfac() {
		if (entities->enpNum[iE] < 0)
			return (-1.0);
		if (!entities->encpBfac[iE])
			entities->load_encpBfac(iE);
		return (*(entities->encpBfac[iE] + iS));
	}
	;

	flt4 ca_x() {
		if (!entities->encpCa[iE])
			entities->load_encpCa(iE);
	...
	XYZ ca_xyz() {
		if (!entities->encpCa[iE])
			entities->load_encpCa(iE);
		XYZ s;
		flt4 *p = entities->encpCa[iE] + iS * 3;
		s.X = *p;
		s.Y = *(p + 1);
		s.Z = *(p + 2);
		return (s);
	...
	int1 code1() {
		if (iM == -1)
			iM = getMon();
		return (monomers->code1(iM));
	}
	;
	int1* code3() {
		if (iM == -1)
			iM = getMon();
		return (monomers->code3(iM));
	}
	;
	int code();
	int nBond() {
		if (iM == -1)
			iM = getMon();
		return (monomers->nBond(iM));
	}
	;
	int2* bonds() {
		if (iM == -1)
			iM = getMon();
		return (monomers->bonds(iM));
	}
	;
	int2 bond1(int iB) {
		if (iM == -1)
			iM = getMon();
		return (monomers->bond1(iM, iB));
	}
	;
	int2 bond2(int iB) {
		if (iM == -1)
			iM = getMon();
		return (monomers->bond2(iM, iB));
	}
	;
	int nAtom() {
		if (iM == -1)
			iM = getMon();
		return (monomers->nAtom(iM));
	}
	;
	int1* atoms() {
		if (iM == -1)
			iM = getMon();
		return (monomers->atoms(iM));
	}
	;
	int1* atom(int iA) {
		if (iM == -1)
			iM = getMon();
		return (monomers->atom(iM, iA));
	}
	;
	int4 prev(int iM) {
		if (iM == -1)
			iM = getMon();
		return (monomers->prev(iM));
	}
	;
	int4 next(int iM) {
		if (iM == -1)
			iM = getMon();
		return (monomers->next(iM));
	}
	;
};
////////////////////////////////////////////////////////////////////
...
public:
	...
	IAtom(ISubentity);
	operator void*() {
		if (!entities->encNXYZ[iE])
			entities->load_encNXYZ(iE);
		if (!entities->enc_xyz_se[iE])
			entities->load_enc_xyz_se(iE);
		if (iA < 0 || iA >= entities->encNXYZ[iE])
			return ((void*) 0);
	...
	void reset() {
		...
		if (iA > 0)
			iA = (iS >= 0 ? *(entities->enc_se_xyz[iE] + iS) : 0);
	}
	;
	void set(int ia) {
		iA = ia + (*(entities->enc_se_xyz[iE] + iS));
	}
	;
	int operator()() {
		return (iA - (*(entities->enc_se_xyz[iE] + iS)));
	}
	;
	int myEntityIndex() {
		return (iE);
	...
	;
	int mySubentityIndex() {
		return (iS);
	}
	;
	flt4 x() {
		if (!entities->encXYZ[iE])
			entities->load_encXYZ(iE);
	...
	;
	flt4 bfac() {
		if (!entities->encBfac[iE])
			entities->load_encBfac(iE);
	...
	XYZ xyz() {
		if (!entities->encXYZ[iE])
			entities->load_encXYZ(iE);
		XYZ s;
		flt4 *p = entities->encXYZ[iE] + iA * 3;
		s.X = *p;
		s.Y = *(p + 1);
		s.Z = *(p + 2);
		return (s);
...
public:
	...
	IMonomer(ISubentity);
	operator void*() {
		return ((void*) (iM < monomers->nMonomers));
	}
	;
	void operator++() {
		if (iM >= 0)
...
public:
	...
	IBond(IAtom);
	operator void*() {
		return ((void*) (iB < monomers->nBond(iM)));
	}
	;
	void operator++() {
		if (iB > 0)
