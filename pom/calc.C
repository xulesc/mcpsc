...
double Torsion::TorsionAngle(IAtom &A, IAtom &B, IAtom &C, IAtom &D) {
	if (A && B && C && D) {
		Point p1((double) A.x(), (double) A.y(), (double) A.z()), p2(
				(double) B.x(), (double) B.y(), (double) B.z()), p3(
				(double) C.x(), (double) C.y(), (double) C.z()), p4(
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 Author: Paul Liu
 ----------------------------------------------------------------------------*/
int Calculate::IsPointOnRamachandranContour(const double &phi,
		const double &psi) {
	int crossings = 0, i;
...
int Calculate::IsRayCrossingSegment(const int &i, const double &phi,
		const double &psi, const torsion rcontour[], const int extent) {
	...
	if (t == 1.0) { // it intersects the end of the line segment; if the next segment
 ...
 *****/
double Calculate::Distance(double x1, double y1, double z1, double x2,
		double y2, double z2) {
	return (sqrt(
...
void Calculate::RamReg(float *phi, float *psi, char **region, int *regno,
		int *regtyp) {
