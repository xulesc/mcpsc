...
public:
	...
	void scratch_align_ent(char *db_tmp_path, char *pdb_dir_name);
	...
	void get_nb(int irep, int icl, DomRep *dr, int *rep_tab, int i_rec,
			int n_rec, int irep2_, int *list_);

	// utils
	void fill_dataset_details(char *dir_name, char **filenames);
	void align_pair(protein_data *protein1, protein_data *protein2,
			int winSize_, double rmsdThr, double rmsdThrJoin);
	void parse_dataset_files_make_entries(char *data_dir, char** filenames,
...
double ce_a1(char *name1, char *name2, XYZ *ca1, XYZ *ca2, int nse1, int nse2,
		int *align_se1, int *align_se2, int &lcmp, double rmsdThrJoin_,
		int isPrint = 1);
...
double dpAlignFrag(double **mat, int nSeq1, int nSeq2, int *align_se1,
		int *align_se2, int &lAlign, double gapI, double gap(int i),
...
public:
	...
	;
	DomNeighbor(const DomNeighbor& dn) {
		deepCopy(dn);
	}
	;
	DomNeighbor& operator=(const DomNeighbor& dn) {
		deepCopy(dn);
	}
	;
	...
	void deepCopy(const DomNeighbor& dn) {
		...
		if (dn.align_rep) {
			...
			for (int is = 0; is < nse_rep; is++)
				align_rep[is] = dn.align_rep[is];
		}
		isSel = dn.isSel;
		p11 = dn.p11;
		p12 = dn.p12;
...
public:
	...
	DomCluster(const DomCluster& dc) {
		deepCopy(dc);
	}
	;
	DomCluster& operator=(const DomCluster& dc) {
		deepCopy(dc);
	}
	;
