...
void CE::scratch_align_ent(int argc, char **argv) {
	...
	///TODO: the temp files created by the program will need to have a core specific name unless they
	///are made in the local tmp of the core. the latter may be a problem due to the small disc space
	///available for the cores.
	...
	if (*chain_id1 != '-') {
		...
		iEnp12 = *i_enp_com.item4(iCom1 + 1);
	}

	int iCom2 = id_com.find(com2);

	if (*chain_id2 != '-') {
		iEnp21 = name_enp.find(ent2);
		iEnp22 = iEnp21 + 1;
	} else {
		iEnp21 = *i_enp_com.item4(iCom2);
		iEnp22 = *i_enp_com.item4(iCom2 + 1);
	}
	...
	if (iEnp11 == -1 || iEnp12 == -1)
		exit(0);

	int isPrint = 2;

	///TODO: this double loop performs CE for all pairs of chains
	///found in the user provided structure files for the two domains.
	///It has been changed to run only the first pair for our tests.
	for (int ienp1 = iEnp11; ienp1 < iEnp11+1; ienp1++)
		for (int ienp2 = iEnp21; ienp2 < iEnp21+1; ienp2++) {
			setText(&name1, pdb_file1);
			addText(&name1, ":");
			...
			int aln_len, gap;
			float rmsd;
			///TODO: this is the real meat of the work.
			z = ce_1(name1, name2, ca1, ca2, nse1, nse2, align_se1, align_se2,
					lcmp, 8, 3.0, 4.0, d_, isPrint, &aln_len, &gap, &rmsd);
			...
			if (lcmp > 0) {
				...
				printf(
						...
						d_[0], d_[1], d_[2], d_[9]);
				printf(
						"     Y2 = (%9.6f)*X1 + (%9.6f)*Y1 + (%9.6f)*Z1 + (%12.6f)\n",
						d_[3], d_[4], d_[5], d_[10]);
				printf(
						"     Z2 = (%9.6f)*X1 + (%9.6f)*Y1 + (%9.6f)*Z1 + (%12.6f)\n",
